...
do
    ...
    -- Auto public-chat removed: no public chat service usage.
    local Lighting = game:GetService("Lighting")
    -- LocalPlayer Locals -- 
    local plr = Players.LocalPlayer
    local mouse = plr:GetMouse()
    -- Wait for the character so the indexing below can't nil-crash on load/respawn
                ...
                if not inv:FindFirstChild("NinjaShuriken1") then
                    ...
                    while bool.antikick and task.wait() do
                        pcall(function()
                            ...
                            task.wait(0.2)
                            if shu and shu:FindFirstChild("StickyPart") and (shu.StickyPart.Position - hrp.Position).Magnitude > 30 then
                                print(4)
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(workspace.ChildAdded:Connect(function(child)
    ...
    Toggles["PCLD_ESP"] = VisualTab:AddToggle({
        ...
        Callback = function(Value)
            if Value then
                ...
                cons["PCLD_ESP_CON"] = workspace.ChildAdded:Connect(function(obj)
                    AddESP(obj)
                end)
            else
...
TarTab:AddButton({
    ...
    Toggles["SpamKeybind"] =  KeybindTab:AddBind({
        ...
        bool.LoopGrabKey = not bool.LoopGrabKey
            if bool.LoopGrabKey then 
                ...
                int.SpamChar = PartToMaybeSpam and PartToMaybeSpam.Parent
                local function runLoopGrabDogPose()
                    cons["LoopGrabCon13"] = RunService.Heartbeat:Connect(function()
                        local SpamChar = int.SpamChar
                        local Torso = SpamChar and SpamChar:FindFirstChild("Torso")
                        local Head = SpamChar and SpamChar:FindFirstChild("Head")
                        if not Torso or not Head then cons:disc("LoopGrabCon13") return end
                        sno(Head)
                        ...
                        local Humanoid = SpamChar:FindFirstChild("Humanoid")
                        ...
                        local LeftArm = SpamChar:FindFirstChild("Left Arm")
                        if LeftArm then LeftArm.CFrame = Torso.CFrame * CFrame.new(-1, 0.5, 0) * CFrame.Angles(math.rad(60), 0, math.rad(-30)) end
                        local RightArm = SpamChar:FindFirstChild("Right Arm")
                        if RightArm then RightArm.CFrame = Torso.CFrame * CFrame.new(1, 0.5, 0) * CFrame.Angles(math.rad(60), 0, math.rad(30)) end
                        local LeftLeg = SpamChar:FindFirstChild("Left Leg")
                        if LeftLeg then LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5, -1, 0) * CFrame.Angles(math.rad(40), 0, 0) end
                        local RightLeg = SpamChar:FindFirstChild("Right Leg")
                        if RightLeg then RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5, -1, 0) * CFrame.Angles(math.rad(40), 0, 0) end
                    end)
                end
                local function runLoopGrabHrpPose()
                    cons["LoopGrabCon13"] = RunService.Heartbeat:Connect(function()
                        local SpamChar = int.SpamChar
                        local Torso = SpamChar and SpamChar:FindFirstChild("Torso")
                        local Head = SpamChar and SpamChar:FindFirstChild("Head")
                ...
                local Head = int.SpamChar and int.SpamChar:FindFirstChild("Head")
                local Torso = int.SpamChar and int.SpamChar:FindFirstChild("Torso")
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(workspace.DescendantAdded:Connect(function(v)
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(RS.GrabEvents.ExtendGrabLine.OnClientEvent:Connect(function(arg1,data)
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(game.Players.PlayerAdded:Connect(function(plr1)
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(game.Players.PlayerRemoving:Connect(function(plr1)
        options, playerMap = BuildPlayerOptions()
        Dropdown1:Refresh(options,true)
    ...
    state.NNHUB_RERUN_CLEANUP_V1:Track(plr.CharacterAdded:Connect(OnCharAdded))
    OrionLib:Init()
    OrionLib:MakeNotification({
