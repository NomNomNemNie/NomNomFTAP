-- XOCU_REGISTER_SCOPE_FIX_V1: wrap generated body to reduce chunk-local register pressure.
do
-- XOCU_RERUN_CLEANUP_V1: disconnect known previous-run globals before creating a new UI/session.
do
    local state = getgenv and getgenv() or _G
    local keys = { "AutoResetCon", "AutoLeaveCon", "BlackHoleWatcher", "MLConn" }
    ...
    if state.cons then
        for key, connection in pairs(state.cons) do
            if connection and connection.Disconnect then pcall(function() connection:Disconnect() end) end
            state.cons[key] = nil
        end
    end
...
-- Unlock MouseBehavior = Enum.MouseBehavior.Default
...
function sendHubLoadedMessage()
	-- Auto public-chat advertisement removed: it leaked the loaded-hub
	-- message (and your username) to everyone in the server. Show a
	-- local-only notify instead so nothing is broadcast.
	pcall(function()
		notify("XOCU", "Loaded", 4)
	end)
end
-- Auto startup chat/local loaded message disabled; call sendHubLoadedMessage() manually if needed.
        ...
        if v then
            _G.AutoResetCon = game:GetService("ReplicatedStorage").GameCorrectionEvents.GameCorrectionsNotify.OnClientEvent:Connect(function(r)
                if r == "Flying" then
                    ...
                    if hum then
                        Library:Notify({ Title = "System", Content = "Resetting to prevent correction", Duration = 4 })
...
DefenseGroup:CreateToggle({
    ...
    Default = false,
    Callback = function(v)
        SetToggleState("Anti Loop Kill", v)
...
DefenseExtra:CreateToggle({
    ...
    Default = false,
    Callback = function(v)
        SetToggleState("Gucci(Invisible)", v)
        if v then
            local blobb
...
updateDropdown()
-- These listeners make the list update automatically; keep only one tracked pair.
