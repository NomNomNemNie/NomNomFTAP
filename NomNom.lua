-- ==========================================================
-- NOMNOM FTAP — Synthesized Hub (Rewrite v2)
-- Private-test / owned-game client utility for "Fling Things And People"
-- ----------------------------------------------------------
-- Synthesized from 6 analyzed source scripts:
--   TSB (movement combat), OverLoad (protections + ESP + blobman),
--   LoopFling (fling engine), UFO (vehicle hijack + massless),
--   VerbalminiLeak (teleport-grab/stack/loop-bring), ChatFTAP (custom chat)
-- ----------------------------------------------------------
-- Engineering: unified connection manager, _G rerun cleanup,
-- respawn-safe character access, throttled loops, no duplicate UI,
-- full teardown on disable/rerun/death.
-- ==========================================================
...
-- ==========================================
-- CONNECTION / INSTANCE MANAGER
-- ==========================================
local Connections = {}   -- named connections (one per key)
local Tasks       = {}   -- spawned task flags
...
local function getToyFolder()
    return w:FindFirstChild(LP.Name .. "SpawnedInToys")
end

-- Safe remote lookups (game-specific; nil if game differs)
...
-- ==========================================
-- ENDGRABEARLY NEUTRALIZE (cleaner anti-grab, from Invisible pattern)
-- Replaces the server EndGrabEarly remote with a dummy so it can't end our grabs.
-- ==========================================
...
if not okUI or not Rayfield then
    warn("[NomNom] Failed to load Rayfield UI")
    return
end
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Synthesized Hub v2",
    Theme = "Amethyst",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "NomNomFTAP",
        FileName = "NomNomConfig",
    },
    KeySystem = false,
})
...
-- ==========================================
-- TAB 2: MOVEMENT (from TSB + fly)
-- ==========================================
local MoveTab = Window:CreateTab("Movement", 4483362458)
MoveTab:CreateSection("Fly")

-- Smooth camera-relative fly (clean BodyVelocity controller)
local flyState = { bv = nil, bg = nil }

local function startFly()
    local hrp = getHRP()
    if not hrp then return end
    stopFly() -- ensure no duplicate movers

    local bv = Instance.new("BodyVelocity")
    bv.Name = "NomNomFlyVel"
...
MoveTab:CreateToggle({
    Name = "Fly (WASD + Space/Shift)", CurrentValue = false, Flag = "Fly",
    Callback = function(v)
        S.Fly = v
...
MoveTab:CreateSlider({
    Name = "Fly Speed", Range = {16, 300}, Increment = 1, Suffix = "studs/s",
...
-- Dash velocity multiplier + free-jump (from The Strongest Battlegrounds source)
...
MoveTab:CreateSlider({
    Name = "Dash Multiplier", Range = {1, 3}, Increment = 0.025,
    CurrentValue = 1.075, Flag = "DashMult",
    Callback = function(v) S.DashMultiplier = v end,
})

MoveTab:CreateToggle({
    Name = "Free Jump (remove NoJump)", CurrentValue = false, Flag = "FreeJump",
...
AddConn("SuperFlingWatcher", w.ChildAdded:Connect(function(child)
    if not S.SuperFling then return end
...
CombatTab:CreateToggle({
    Name = "Super Fling (on grab release)", CurrentValue = false, Flag = "SuperFling",
    Callback = function(v) S.SuperFling = v end,
})

CombatTab:CreateSlider({
    Name = "Fling Strength", Range = {100, 5000}, Increment = 50,
    CurrentValue = 850, Flag = "FlingPower",
    Callback = function(v) S.FlingStrength = v end,
})

-- --- MASSLESS GRAB (from UFO) ---
CombatTab:CreateSection("Massless Grab")

CombatTab:CreateToggle({
    Name = "Massless Grab (Player & Object)", CurrentValue = false, Flag = "Massless",
    Callback = function(v)
        S.Massless = v
        if v then
            AddConn("Massless", w.ChildAdded:Connect(function(r)
                ...
                while w:FindFirstChild("GrabParts") and S.Massless do
                    ...
                    if dp then
                        ...
                        if ap then
                            ap.Responsiveness = S.MasslessSense
                            ap.MaxForce = math.huge
                            ap.MaxVelocity = math.huge
                        end
                        if ao then
                            ao.Responsiveness = S.MasslessSense
...
CombatTab:CreateInput({
    Name = "Massless Sense", CurrentValue = "30",
    PlaceholderText = "Enter sense value", RemoveTextAfterFocusLost = false,
    Flag = "MasslessSense",
    Callback = function(t) local n = tonumber(t); if n and n > 0 then S.MasslessSense = n end end,
})

-- --- TELEPORT & GRAB / STACK / LOOP-BRING (from VerbalminiLeak) ---
CombatTab:CreateSection("Teleport & Bring")

local grabbed = {}        -- {root, connection, humanoid}
...
local function grabPlayer(target)
    ...
    local destCF
    if S.StackMode then
        destCF = origCF * CFrame.new(0, S.GrabHeight * (#grabbed + 1), 0)
    else
        destCF = origCF * CFrame.new(math.random(-5, 5), S.GrabHeight, math.random(-5, 5))
    end
    tHRP.CFrame = destCF
    ...
    for _, d in ipairs(grabbed) do
        ...
        if d.root and d.humanoid then
            pcall(function()
                ...
                d.humanoid.Sit = false
                d.humanoid.WalkSpeed = 16
...
local function bringTargets(targets)
    ...
    for _, plr in ipairs(targets) do
        if plr ~= LP and plr.Character then grabPlayer(plr) end
    end
        ...
        if myHRP then
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= LP and plr.Character then
                    ...
                    if tHRP and (myHRP.Position - tHRP.Position).Magnitude <= 70 then
...
CombatTab:CreateToggle({
    Name = "Stack Up Mode", CurrentValue = false, Flag = "StackMode",
    Callback = function(v) S.StackMode = v; if not v then releaseAll() end end,
})

CombatTab:CreateToggle({
    Name = "Loop Bring", CurrentValue = false, Flag = "LoopBring",
    Callback = function(v)
        S.LoopBring = v
        if v then
            ...
            task.spawn(function()
                while S.LoopBring and Tasks.LoopBring do
                    bringTargets(collectTargets())
                    task.wait(S.BringDelay)
                end
            end)
        else
            Tasks.LoopBring = false
            releaseAll()
        end
    end,
})
...
CombatTab:CreateDropdown({
    Name = "Bring Mode", Options = {"all", "nearby", "whitelist"},
    CurrentOption = {"all"}, Flag = "BringMode",
    Callback = function(o) S.BringMode = type(o) == "table" and o[1] or o end,
})

CombatTab:CreateSlider({
    Name = "Bring Delay", Range = {0.5, 10}, Increment = 0.1, Suffix = "s",
    CurrentValue = 1, Flag = "BringDelay",
    Callback = function(v) S.BringDelay = v end,
})

CombatTab:CreateButton({
    Name = "Bring All (once)",
    Callback = function() bringTargets(Players:GetPlayers()) end,
})

-- Teleport-grab keybind (Y) — single target under mouse
...
-- --- LOOP FLING (from LoopFling.lua — predictive + LOS + ownership monitor) ---
CombatTab:CreateSection("Loop Fling")

local fling = {
    decoy = nil, target = nil, conn = nil, targetIndex = 1,
...
local function setupFling(d)
    ...
    local hrp = d:FindFirstChild(partName)
    if not hrp then return end
    ...
    fling.conn = RunService.Heartbeat:Connect(function()
        ...
        if fling.target and fling.target.Character then
            ...
            if tHRP then
                ...
                local result = w:Raycast(hrp.Position, dir, params)
...
local function spawnFlingDecoy()
    ...
    if not myHRP or not spawnToy then return end
    pcall(function()
        spawnToy:InvokeServer(S.SelectedToy, myHRP.CFrame * CFrame.new(5, 0, 5), Vector3.new(0, 33, 0))
    end)
end
...
local function handleDecoy(d)
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        if tag and tag:IsA("StringValue") and tag.Value == LP.Name then
            success = true
            conn:Disconnect()
            d:SetAttribute("NomNomOwned", true)
            fling.decoy = d
    ...
    if not fling.decoy or not fling.decoy.Parent then
        ...
        if folder then
            for _, t in ipairs(folder:GetChildren()) do
                if toyMap[t.Name] and t:GetAttribute("NomNomOwned") then
                    fling.decoy = t
                    return
                end
            end
...
local function updateFlingTargets()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP then p:SetAttribute("NomNomTarget", S.LoopFling) end
    end
end

CombatTab:CreateToggle({
    Name = "Loop Fling (all players)", CurrentValue = false, Flag = "LoopFling",
    Callback = function(state)
        S.LoopFling = state
        updateFlingTargets()
        if state then
            ...
            local destroyToy = remote({"MenuToys", "DestroyToy"})
            if fling.decoy and fling.decoy.Parent and destroyToy then
                pcall(function() destroyToy:FireServer(fling.decoy) end)
            end
...
ProtTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            ...
            Tasks.AntiGrab = false
        end
    end,
})

ProtTab:CreateToggle({
    Name = "Anti Explode", CurrentValue = false, Flag = "AntiExplode",
    Callback = function(v)
        S.AntiExplode = v
        if v then
            ...
            RemoveConn("AntiExplode")
        end
    end,
})

ProtTab:CreateToggle({
    Name = "Anti Fire", CurrentValue = false, Flag = "AntiFire",
    Callback = function(v)
        S.AntiFire = v
        if v then
            local extPart
            ...
            AddConn("AntiFire", RunService.Heartbeat:Connect(function()
                ...
                if hrp:FindFirstChild("FireLight") or hrp:FindFirstChild("FireParticleEmitter") then
                    ...
                    extPart.CFrame = CFrame.new(origPos)
                end
            end))
        else
            RemoveConn("AntiFire")
...
ProtTab:CreateToggle({
    Name = "Anti Blobman", CurrentValue = false, Flag = "AntiBlobman",
    Callback = function(v)
        S.AntiBlobman = v
        if v then
...
local function spawnShurikens(count)
    local hrp = getHRP()
    local spawnToy = remote({"MenuToys", "SpawnToyRemoteFunction"})
    if not hrp or not spawnToy then return end
    local cf = hrp.CFrame * CFrame.new(0, 3, 0)
    for _ = 1, count do
        pcall(function() spawnToy:InvokeServer("NinjaShuriken", cf, Vector3.new(0, 0, 0)) end)
        task.wait(0.05)
    end
end
    ...
    for i = 1, 10 do
        pcall(function()
            ...
            if shur then
                ...
                sticky:FireServer(shur.StickyPart, target, CFrame.Angles(0, 0, 0))
            end
        end)
    end
...
pcall(function()
    ...
    for _, c in ipairs(f:GetChildren()) do
        if c.Name:match("Hitbox") then table.insert(ufoHitboxes, c) end
    end
end)
...
AddConn("UFOHitbox", RunService.RenderStepped:Connect(function(dt)
    ...
    elseif S.UFOFollow then
        for _, hb in ipairs(ufoHitboxes) do
            if hb and hb.Parent then
                hb.CFrame = CFrame.new(hrp.Position + Vector3.new(0, S.UFOHeight + 3, 0))
            end
        end
    end
end))
    ...
    local function setup(char)
        ...
        task.spawn(function()
            ...
            while char and char.Parent and hl.Parent do
                ...
                hl.FillColor = col
                hl.OutlineColor = col
                bill.Enabled = S.ESP and S.ESPNames
...
ESPTab:CreateToggle({ Name = "Show Names & Photos", CurrentValue = false, Flag = "ESPNames",
    Callback = function(v) S.ESPNames = v end })
ESPTab:CreateToggle({ Name = "Rainbow Mode", CurrentValue = false, Flag = "ESPRainbow",
    Callback = function(v) S.ESPRainbow = v end })
ESPTab:CreateColorPicker({ Name = "ESP Color", Color = Color3.fromRGB(0, 255, 100), Flag = "ESPColor",
    Callback = function(v) S.ESPColor = v end })
ESPTab:CreateButton({ Name = "Force Refresh ESP",
    Callback = function()
        for _, p in ipairs(Players:GetPlayers()) do applyESP(p) end
        notify("ESP refreshed", "NomNom", 2)
    end })

-- ==========================================
-- TAB 7: CHAT (from ChatFTAP — via ExtendGrabLine)
-- ==========================================
...
local ChatMain = Instance.new("Frame")
ChatMain.Parent = ChatGui
ChatMain.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
ChatMain.BorderSizePixel = 0
ChatMain.Position = UDim2.new(0.5, -175, 0.5, -150)
ChatMain.Size = UDim2.new(0, 350, 0, 300)
ChatMain.ClipsDescendants = true
ChatMain.Active = true
ChatMain.Draggable = true
Instance.new("UICorner", ChatMain).CornerRadius = UDim.new(0, 20)

local ChatTop = Instance.new("Frame", ChatMain)
ChatTop.BackgroundColor3 = Color3.fromRGB(28, 28, 35)
ChatTop.BorderSizePixel = 0
ChatTop.Size = UDim2.new(1, 0, 0, 45)
Instance.new("UICorner", ChatTop).CornerRadius = UDim.new(0, 20)

local ChatTitle = Instance.new("TextLabel", ChatTop)
ChatTitle.BackgroundTransparency = 1
ChatTitle.Position = UDim2.new(0, 16, 0, 0)
ChatTitle.Size = UDim2.new(0, 200, 1, 0)
ChatTitle.Font = Enum.Font.GothamBold
ChatTitle.Text = "NomNom Chat"
...
ChatClose.BackgroundColor3 = Color3.fromRGB(45, 45, 55)
ChatClose.Position = UDim2.new(1, -35, 0.5, -12)
ChatClose.Size = UDim2.new(0, 24, 0, 24)
ChatClose.Font = Enum.Font.GothamBold
ChatClose.Text = "X"
ChatClose.TextColor3 = Color3.fromRGB(255, 255, 255)
ChatClose.TextSize = 14
...
SendBtn.Size = UDim2.new(0, 45, 0, 30)
...
local function makeBubble(user, msg, isSelf)
    ...
    n.Text = user
    n.TextColor3 = isSelf and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(100, 200, 255)
    n.TextSize = 12
    n.TextXAlignment = Enum.TextXAlignment.Left

    local m = Instance.new("TextLabel", bubble)
    m.BackgroundTransparency = 1
    m.Position = UDim2.new(0, 0, 0, 18)
...
local function addMessage(user, msg, isSelf)
    ...
    for _, e in ipairs(ChatHistory) do
        makeBubble(e.user, e.msg, e.isSelf).Parent = HistoryFrame
    end
    scrollBottom()
end
...
local function sendMessage()
    ...
    pcall(function()
        ...
        if ext then
            ext:FireServer("CUSTOMMSG:" .. LP.DisplayName .. ":" .. message)
            MessageBox.Text = ""
            addMessage(LP.DisplayName, message, true)
        end
    end)
end
        ...
        AddConn("ChatRecv", ext.OnClientEvent:Connect(function(...)
            for _, v in ipairs({ ... }) do
                if typeof(v) == "string" and v:sub(1, 10) == "CUSTOMMSG:" then
                    ...
                    if #parts >= 3 then
                        addMessage(parts[2], table.concat(parts, ":", 3), false)
                    end
                    return
                end
            end
...
AddConn("ChatF8", UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == Enum.KeyCode.F8 then
        ...
        if ChatGui.Enabled then scrollBottom() end
    end
end))
...
SettingsTab:CreateInput({
    Name = "Player Name", CurrentValue = "",
    PlaceholderText = "Enter player name", RemoveTextAfterFocusLost = false,
    Flag = "WhitelistInput",
...
SettingsTab:CreateButton({ Name = "Add to Whitelist",
    Callback = function()
        if whitelistInput ~= "" and not table.find(S.Whitelist, whitelistInput) then
            table.insert(S.Whitelist, whitelistInput)
            notify("Added: " .. whitelistInput, "Whitelist")
        end
    end })
SettingsTab:CreateButton({ Name = "Remove from Whitelist",
    Callback = function()
        for i, name in ipairs(S.Whitelist) do
            if name == whitelistInput then
                table.remove(S.Whitelist, i)
                notify("Removed: " .. whitelistInput, "Whitelist")
                break
            end
        end
    end })
SettingsTab:CreateButton({ Name = "Show Whitelist",
    Callback = function()
        notify(#S.Whitelist > 0 and table.concat(S.Whitelist, ", ") or "Empty", "Whitelist", 5)
    end })
...
SettingsTab:CreateParagraph({ Title = "NomNom FTAP v2",
    Content = "Synthesized from TSB, OverLoad, LoopFling, UFO, VerbalminiLeak, ChatFTAP.\n\nTabs: Main, Movement, Combat, Protection, Vehicles, ESP, Chat, Settings.\nF8 = Chat. Y = Teleport-grab target under mouse." })
...
local function Cleanup()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Character then
            local e = p.Character:FindFirstChild("NomNom_ESP")
            local t = p.Character:FindFirstChild("NomNom_Tag")
            if e then e:Destroy() end
            if t then t:Destroy() end
        end
        if p ~= LP then p:SetAttribute("NomNomTarget", nil) end
    end
    -- chat GUI
...
-- STARTUP
-- ==========================================
notify("8 tabs ready. F8 = Chat, Y = Teleport-grab.", "NomNom FTAP Loaded", 5)
print("[NomNom] FTAP v2 loaded. _G.NomNomFTAP.Cleanup() to unload.")